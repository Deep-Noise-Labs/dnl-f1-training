...
def create_dataloader_from_config(dataset_config, batch_size, sample_size, sample_rate, audio_channels=2, num_workers=4):
    ...
    if dataset_type == "audio_dir":
        ...
        # GCS-native streaming dataset: WAV + JSON pairs in a GCS bucket.
        # Requires google-cloud-storage and Application Default Credentials.
        # Source: stable_audio_tools/data/gcs_dataset.py
        from .gcs_dataset import GCSDataset
        gcs_datasets = []
        for gcs_cfg in dataset_config.get("datasets", []):
            gcs_bucket = gcs_cfg.get("bucket", None)
            gcs_prefix = gcs_cfg.get("prefix", "")
            assert gcs_bucket is not None, "'bucket' must be set for gcs dataset type"
            custom_metadata_fn = None
            custom_metadata_module_path = gcs_cfg.get("custom_metadata_module", None)
            if custom_metadata_module_path is not None:
                spec = importlib.util.spec_from_file_location("metadata_module", custom_metadata_module_path)
                metadata_module = importlib.util.module_from_spec(spec)
                spec.loader.exec_module(metadata_module)
            ...
            gcs_datasets.append(
                GCSDataset(
                    ...
                    force_channels=force_channels,
                    custom_metadata_fn=custom_metadata_fn,
                    max_items=gcs_cfg.get("max_items", None),
                )
            )
        if len(gcs_datasets) == 1:
